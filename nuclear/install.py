...
def install_all() -> int:
    ...
    if not package_dict:
        log.error('no packages to be installed')
        return -1

    for package_name, package_info in package_dict.items():
        version = package_info[1]
        log.info(f"installing {package_name} with version {version}")
...
def install_single_package(package_name: str, version: str) -> int:
    ...
    if not get.check_repo(user, repo):
        log.error(
            f"The wanted package '{repo}' from {user} couldn't be found on GitHub")
        return -1
    ...
    if tar_addr is not None:
        ...
        try:
            makedirs(get_dir.get_dir_name(
                tar_addr, version=version), exist_ok=True)
